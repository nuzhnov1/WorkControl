...
import com.nuzhnov.workcontrol.core.data.api.util.Response.Failure.*
...
@PublishedApi
internal fun <T> Result<T>.toResponseResult() = this.fold(
...
private fun HttpException.toHttpFailure() = when (code()) {
    in 400..499 -> this.toHttpClientFailure()
    in 500..599 -> this.toHttpServerFailure()
    else -> UnknownError(cause = this)
}

private fun HttpException.toHttpClientFailure() = when (code()) {
...
private fun HttpException.toHttpServerFailure() = when (code()) {
...
private fun IOException.toNetworkError() = when (this) {
