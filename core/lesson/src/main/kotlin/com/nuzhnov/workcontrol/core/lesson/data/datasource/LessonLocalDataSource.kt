...
internal class LessonLocalDataSource @Inject constructor(
    ...
    suspend fun saveLessonModels(vararg lessonModel: LessonModel): Result<Unit> =
    ...
    suspend fun updateLesson(lessonModel: LessonModel): Result<Unit> =
        safeExecute {
            removeLesson(lessonModel.lessonEntity).getOrThrow()
            addLesson(lessonModel).getOrThrow()
        }

    suspend fun removeLesson(lessonEntity: LessonEntity): Result<Unit> =
        safeExecute(context = coroutineDispatcher) {
            lessonDAO.delete(lessonEntity)
        }

    suspend fun startLesson(lessonID: Long): Result<Unit> =
        safeExecute(context = coroutineDispatcher) {
    ...
    suspend fun finishLesson(lessonID: Long): Result<Unit> =
        safeExecute(context = coroutineDispatcher) {
            val session = appPreferences.getSession()
